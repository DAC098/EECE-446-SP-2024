...
void close_app() { // Named to avoid conflict with exit()
    if (sock != -1) {
...
int main(int argc, char *argv[]) {
    ...
    peer_id = atoi(argv[3]); // Convert peer ID from argumentw

    // Initialize networking and run print_options()
    // Networking initialization code here...
